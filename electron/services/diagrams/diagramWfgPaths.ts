...
import { WFG_FILE_EXTENSION } from '@shared/documentPackage/constants'
import { diagramTitleBase } from '../../../src/modules/library/diagrams/lib/diagramHomeUtils'
...
export function diagramWfgFileName(title: string): string {
  return `${sanitizeDiagramWfgBaseName(title)}${WFG_FILE_EXTENSION}`
}
...
export function findDiagramWfgPath(mediaDir: string, fileId: string): string | null {
  ...
  for (const name of readdirSync(dir)) {
    if (name.toLowerCase().endsWith(WFG_FILE_EXTENSION)) {
