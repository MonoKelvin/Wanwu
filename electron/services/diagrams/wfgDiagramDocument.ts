...
import { MANIFEST_ENTRY_PATH, WFG_PATHS } from '@shared/documentPackage'
import type { DiagramContent, DiagramPage } from '../../../src/shared/types/diagrams'
import { WanwuDocumentPackage } from '../documentPackage/WanwuDocumentPackage'
import {
  openPackageFromFolder,
  saveAllEntriesToFolder,
  saveDirtyEntriesToFolder
} from '../documentPackage/fsStore'
import { openPackageFromZip, savePackageToZip } from '../documentPackage/zipStore'
...
export class WfgDiagramDocument {
  private constructor(
    ...
    private readonly pkg: WanwuDocumentPackage
  ) {}
  ...
  readContent(): DiagramContent {
    const metaText = this.pkg.getEntryText(WFG_PATHS.meta)
    if (!metaText) {
  ...
  replaceContent(content: DiagramContent): void {
    this.pkg.setEntryJson(WFG_PATHS.meta, toMetaFile(content))
    ...
    for (const page of content.pages) {
      const path = WFG_PATHS.page(page.id)
  ...
  writePage(page: DiagramPage): void {
    this.pkg.setEntryJson(WFG_PATHS.page(page.id), page)
  }

  deletePageEntry(pageId: string): void {
    this.pkg.deleteEntry(WFG_PATHS.page(pageId))
  }
  ...
  writeMeta(partial: Partial<WfgDiagramMetaFile>): void {
    ...
    this.pkg.setEntryJson(WFG_PATHS.meta, { ...base, ...partial })
    if (partial.title) this.pkg.updateTitle(partial.title)
  }

  writeAsset(assetId: string, ext: string, data: Buffer): void {
    this.pkg.setEntryBuffer(WFG_PATHS.asset(assetId, ext), data, {
  ...
  getPackage(): WanwuDocumentPackage {
    return this.pkg
  }
}
