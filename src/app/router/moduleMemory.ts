...
import { collectPathMemoryContributors } from '@app/modules/pathMemoryRegistry'
...
export function setupModulePathMemory(router: Router) {
  router.beforeEach((to, from) => {
    for (const contributor of collectPathMemoryContributors()) {
      const result = contributor.beforeEach?.(to, from)
