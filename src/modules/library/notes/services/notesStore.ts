...
export const useNotesStore = defineStore('notes', () => {
  ...
  async function loadAll(): Promise<NoteItem[]> {
    bindRemoteSync()
    loading.value = true
    try {
  ...
  async function createNote(input?: NoteCreateInput) {
    const created = await window.wanwu.notes.createNote(input ?? { color: 'yellow' })
