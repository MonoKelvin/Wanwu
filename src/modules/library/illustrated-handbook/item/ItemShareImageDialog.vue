...
import {
  ...
  type ShareTarget,
  type ShareTargetId
} from '@modules/library/illustrated-handbook/item/constants/shareTargets'
...
import { buildItemShareText } from '@modules/library/illustrated-handbook/item/utils/buildItemCopyText'
import { buildQzoneShareUrl, buildWeiboShareUrl } from '@modules/library/illustrated-handbook/item/utils/buildSocialShareUrl'
import { captureDetailLongImage } from '@modules/library/illustrated-handbook/item/utils/captureDetailLongImage'
import {
  buildDetailShareHtml,
  type DetailGallerySlide,
  type ShareExportFormat
} from '@modules/library/illustrated-handbook/item/utils/exportDetailHtml'
...
async function renderPreview() {
  ...
  try {
    ...
    error.value = format.value === 'html' ? '���� HTML ʧ�ܣ����Ժ�����' : '���ɳ�ͼʧ�ܣ����Ժ�����'
  } finally {
...
async function shareViaSocial(target: 'weibo' | 'qqzone') {
  ...
  try {
    ...
    emit(
      'toast',
      `�Ѵ�${target === 'weibo' ? '΢��' : 'QQ �ռ�'}����ҳ������ ${SHARE_TEMP_EXPIRE_LABEL} ��ʧЧ���뾡����ɷ���`
    )
    return true
  } catch {
    emit('toast', '����ʧ�ܣ�����ȫ����Ӧ�ú�����')
    return false
  } finally {
    loadingTarget.value = null
  }
...
<template>
  ...
  >
    <p class="ww-share-image-dialog__hint">
      ���� PNG / JPEG ��ͼ�� HTML �ļ���HTML ��֧�ֱ��浽���ػ�ϵͳ������罻ƽ̨��ʹ�� PNG / JPEG��
    </p>
    ...
    <div class="ww-share-image-dialog__preview">
      <div class="ww-share-image-dialog__preview-body">
        ...
        />
        <p v-else-if="!generating" class="ww-share-image-dialog__status">����Ԥ��</p>
      </div>
      <div v-if="generating" class="ww-share-image-dialog__preview-overlay" aria-live="polite">
        {{ format === 'html' ? '�������� HTML��' : '������Ⱦ��ͼ��' }}
      </div>
    </div>
    ...
    <section class="ww-share-image-dialog__targets-wrap" aria-label="�����ʽ">
      <p class="ww-share-image-dialog__targets-title">������</p>
      ...
      <p v-if="showUploadHint" class="ww-share-image-dialog__upload-hint">
        �����ϴ�����ʱͼ����Litterbox�������� {{ SHARE_TEMP_EXPIRE_LABEL }} ���Զ�ʧЧ���뾡����ɷ����
      </p>
    </section>
  </WwGlassDialog>
</template>
