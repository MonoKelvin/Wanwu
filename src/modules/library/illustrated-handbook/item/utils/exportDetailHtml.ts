...
import { htmlExportStyles, resolveShareTheme } from '@modules/library/illustrated-handbook/item/utils/shareExportTheme'
...
export async function buildDetailShareHtml(options: {
  ...
  const specsHtml =
    specEntries.length > 0
      ? `<div class="spec-block"><h2 class="section-label">������</h2><dl class="specs">${specEntries
          .map(
            ([k, v]) =>
  ...
  const descHtml = item.description?.trim()
    ? `<section class="desc"><h2 class="section-label">��ϸ����</h2><div class="prose">${renderMarkdown(item.description)}</div></section>`
    : ''
...
<body>
  <div class="page">
    ...
    ${descHtml}
    <p class="footer-note">�� Wanwu ���� �� ${escapeHtml(new Date().toLocaleString('zh-CN'))}</p>
  </div>
</body>
</html>`
}
