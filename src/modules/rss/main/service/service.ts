...
import { probeRssFeedUrl } from './probe'
import { ensureRssSchema, initRssBaseTables, validateFeedTitle, validateFeedUrl } from './schema'
import { seedDefaultRssFeeds } from './defaults'
import { extractEntryImageUrl, iconUrlFromFeedMeta } from './entryMedia'
import { assessFeedQuality, evaluateFeedHealth } from './health'
...
export class RssService {
  ...
  private readonly rssDb: Database.Database

  constructor(basePath: string) {
    const root = ensureWanwuDataLayout(basePath)
    const layout = getWanwuPathLayout(root)
    this.rssDb = new Database(rssDbFile(layout))
    initRssBaseTables(this.rssDb)
  ...
  searchQuickAccess(query: string, limit: number): QuickAccessHit[] {
    ensureRssSchema(this.rssDb)
    const like = `%${query.trim()}%`
    const rssRows = this.rssDb
      .prepare(
