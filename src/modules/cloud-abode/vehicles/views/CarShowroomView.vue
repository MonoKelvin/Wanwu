...
import { CLOUD_ABODE_SHOWROOM } from '@modules/cloud-abode/vehicles/config/showroomAssets'
...
async function onReady(r: SceneRenderer) {
  ...
  try {
    ...
    if (props.owned) {
      const cloud = await window.wanwu.cloudAbode.getVehicleCustomization(props.slug)
      if (cloud && typeof cloud.bodyColor === 'string') {
        garage.patchConfig({
          bodyColor: String(cloud.bodyColor),
          wheelId: typeof cloud.wheelId === 'string' ? cloud.wheelId : undefined,
          liveryId: typeof cloud.liveryId === 'string' ? cloud.liveryId : undefined
        })
      }
    }
...
watch(
  () => garage.config,
  (cfg) => {
    if (!props.owned || !cfg) return
    void window.wanwu.cloudAbode.saveVehicleCustomization(props.slug, {
      bodyColor: cfg.bodyColor,
      wheelId: cfg.wheelId,
      liveryId: cfg.liveryId
    })
  },
  { deep: true }
)
