...
import type { MusicKugouLoginStatus } from '@shared/types/music'
...
const status = ref<MusicKugouLoginStatus>({ loggedIn: false, loginType: 'none' })
const qrImage = ref('')
const qrKey = ref('')
const phone = ref('')
const captcha = ref('')
...
async function refreshStatus() {
  status.value = await window.wanwu.music.kugouGetLoginStatus()
}
...
async function startQrLogin() {
  ...
  message.value = null
  try {
    const qr = await window.wanwu.music.kugouLoginQrKey()
    qrKey.value = qr.key
...
async function pollQr() {
  ...
  try {
    const res = await window.wanwu.music.kugouLoginQrCheck(qrKey.value)
    if (res.status === 4) {
      stopPoll()
...
async function sendCaptcha() {
  ...
  message.value = null
  try {
    await window.wanwu.music.kugouSendCaptcha(phone.value.trim())
    finishInfo('验证码已发送')
  } catch (e) {
...
async function loginPhone() {
  ...
  message.value = null
  try {
    await window.wanwu.music.kugouLoginPhone(phone.value.trim(), captcha.value.trim())
    loginComplete.value = true
    await refreshStatus()
    finishSuccess()
  } catch (e) {
...
async function loginCookie() {
  ...
  try {
    const token =
      cookieText.value.includes('token=')
        ? (cookieText.value.match(/token=([^;]+)/)?.[1] ?? cookieText.value.trim())
        : cookieText.value.trim()
    status.value = await window.wanwu.music.kugouLoginCookie(token)
    loginComplete.value = true
    finishSuccess()
  } catch (e) {
    finishError(e instanceof Error ? e.message : 'Token 无效')
  }
}

async function logout() {
  await window.wanwu.music.kugouLogout()
  ...
  >
    <div class="ww-music-login">
      ...
      <div v-else-if="!status.loggedIn" key="cookie" class="ww-music-login__panel ww-music-login__form">
        <div class="ww-music-login__field">
          <InputText v-model="cookieText" placeholder="粘贴 token 或完整 Cookie" class="w-full" />
        </div>
