...
onMounted(async () => {
  ...
  const toplistId = browseId.includes(':') ? browseId.split(':').pop() ?? browseId : browseId
  loading.value = true
  try {
    tracks.value = await window.wanwu.music.getToplistTracks(toplistId, 100)
    title.value = route.query.title ? String(route.query.title) : '排行榜'
  } finally {
    loading.value = false
  }
})
...
<template>
  <MusicScrollBody>
    <div class="ww-music-content-shell">
      <MusicPageHeading :title="title" subtitle="榜单详情" />
      ...
      <p v-else class="ww-music-state-hint">榜单为空或无法加载。</p>
    </div>
  </MusicScrollBody>
</template>
